//using UintZZZ = System.UInt32;
using UintZZZ = ConsoleApp1.UIntX;
using ByteZZZ = System.Byte;
...
namespace ConsoleApp1
{
    ...
    public class MD5
    {
        ...
        public static UIntX leftRotate(UIntX x, int c)
        {
           return x.LeftRotate(c);
        }
        ...
        public static string Calculate(ByteZZZ[] input)
        {
            UintZZZ a0 = 0x67452301;   // A
            UintZZZ b0 = 0xefcdab89;   // B
            UintZZZ c0 = 0x98badcfe;   // C
            UintZZZ d0 = 0x10325476;   // D
            ...
            for (int i = 0; i < processedInput.Length / 64; ++i)
            {
                ...
                uint[] M = new uint[16];
                for(int j = 0; j < 16; ++j)
                    M[j] = ToUInt(processedInput, i, j);

                // initialize round variables
                UintZZZ A = a0, B = b0, C = c0, D = d0, F;
                uint g = 0;
        ...
        private static uint ToUInt(byte[] processedInput, int i, int j)
        {
            return BitConverter.ToUInt32(processedInput, (i * 64) + (j * 4));
        }
